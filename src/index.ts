...
export function routingControllersToSpec(
  ...
  const filteredSchemas: { [p: string]: oa.SchemaObject } = Object.fromEntries(
    Object.entries(schemas)
      .filter(([, schema]) => !('$ref' in schema))
      .map(([key, schema]) => [key, schema as oa.SchemaObject])
  )
  const spec = getSpec(routes, filteredSchemas)
